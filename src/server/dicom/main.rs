...
fn run(scu_stream: TcpStream, args: &App) -> Result<(), Whatever> {
    ...
    let mut options = dicom_ul::association::ServerAssociationOptions
        ::new()
        .accept_any()
                ...
                match pdu {
                    Pdu::PData { ref mut data } => {
                        ...
                        if data[0].value_type == PDataValueType::Data && !data[0].is_last {
                            ...
                            obj
                                .write_dataset_with_ts(&mut obj_data, &ts)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing::subscriber
        ...
        .unwrap_or_else(|e| {
            eprintln!("Could not set up global logger: {}", snafu::Report::from_error(e));
        });

    let listen_addr = SocketAddrV4::new(Ipv4Addr::from(0), args.port);
    let listener = TcpListener::bind(listen_addr)?;
    info!("{} listening on: tcp://{}", &args.calling_ae_title, listen_addr);
